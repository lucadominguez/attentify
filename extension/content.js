...
(function () {
  ...
  let suppressSelectors = new Set();       // selectors the user flagged "not a distraction" here — scanner must skip (persisted across reloads)
  const sessionUnhidden = new Set();       // selectors the user unhid/restored/dismissed THIS session — the auto-scanner must not re-hide them, even before the persisted suppression propagates (fixes "I dismiss it and it re-blocks")
  ...
  function scanForDistractions() {
    ...
    for (const el of regions) {
      ...
      // The user dismissed/restored it this session — do not re-hide it out from under them.
      if (sessionUnhidden.has(selector)) continue;
      seen.add(selector);
      cands.push(scored);
  ...
  function resetBehavior(navType) {
    ...
    // A new page (SPA nav) starts a fresh dwell/engagement accounting for the re-report gate.
    fgTotal = 0; lastDwellReport = 0;
  }
  ...
  let fgTotal = 0;         // total foreground ms on this page (across flushes)
  let lastDwellReport = 0; // throttle for mid-session context re-reports
  ...
  function fgFlush() {
    ...
    if (fgPending >= 1000) {
      ...
      // Mid-session dwell re-report: once real eyes-on time and engagement have built up on a
      // page, re-report the context (throttled to ~once/minute) so the daemon classifier sees
      // ACCRUED dwell/scroll, not just the near-zero snapshot at navigation. Re-assessment is
      // cache-cheap (no extra AI call), and this is what lets sustained engagement escalate an
      // ambiguous page. No-op when there is no daemon — the extension stays fully standalone.
      const eng = fgTotal > 45000 || behavior.maxScroll > 0.5 || behavior.recoClicks > 0;
      if (eng && Date.now() - lastDwellReport > 60000) {
        lastDwellReport = Date.now();
        reportContext('dwell');
      }
    }
    fgResume();
  ...
  chrome.runtime.onMessage.addListener((msg, _s, sendResponse) => {
    ...
    if (msg.type === 'unhide:adhoc') {
      ...
      // Remember the user's choice for this session so the auto-scanner does not immediately
      // re-detect and re-hide the very thing they just dismissed. (A permanent "not a
      // distraction?" also persists via autoHideSuppress; this covers the gap before that
      // async write propagates, and makes a plain "Restore" actually stick for the session.)
      for (const s of rm) sessionUnhidden.add(s);
