...
// ── How Attentify decides (transparent reasoning, ported from the extension's Logic page) ──
// The same "here is the logic, in the open" showcase the browser extension carries: the
// distraction taxonomy it works from, plus the decision principles the daemon classifier
// actually follows. Static/educational — no account or data needed, so it renders signed out.
const DECISION_TAXONOMY = [
  ...
  { key: 'Algorithmic home feeds', weight: 0.60, examples: 'X, Facebook, Reddit, Instagram home', why: 'An endless personalised stream with no goal state. You did not come for a specific thing, so there is no point where you are done.' },
  { key: 'Recommendation rails', weight: 0.40, examples: 'YouTube "Up next" and "Recommended for you"', why: 'Sidebars that turn one purposeful visit into a chain of one-more-click suggestions.' },
]
...
function HowItDecides(): React.ReactElement {
  const { colors } = useTheme()
  return (
    <>
      ...
      <div className="section-panel p-3.5 space-y-3">
          ...
          <div className="space-y-2">
            {DECISION_TAXONOMY.map((t) => (
              <div key={t.key} className="rounded-lg px-3 py-2" style={{ background: colors.inputBg, border: `1px solid ${colors.border}` }}>
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[12px] font-semibold" style={{ color: colors.textPrimary }}>{t.key}</span>
                  <div className="flex items-center gap-1.5">
                    <div className="h-1.5 rounded-full overflow-hidden" style={{ width: 48, background: colors.border }}>
                      <div className="h-full rounded-full" style={{ width: `${Math.round(t.weight * 100)}%`, background: colors.warning }} />
                    </div>
        ...
        <div className="space-y-2">
          {DECISION_PRINCIPLES.map((p) => (
            <div key={p.title} className="rounded-lg px-3 py-2" style={{ background: colors.cardBg, border: `1px solid ${colors.border}`, borderLeftWidth: 2, borderLeftColor: colors.accent }}>
              <p className="text-[12px] font-semibold" style={{ color: colors.textPrimary }}>{p.title}</p>
              <p className="text-[11.5px] leading-snug mt-0.5" style={{ color: colors.textSecondary }}>{p.body}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="max-w-4xl mx-auto px-4 py-4 space-y-4">
          ...
          {loading ? (
            ...
            <>
              ...
              {/* How Attentify decides — the transparent taxonomy + decision principles, at
                  parity with the browser extension's Logic page. */}
              <HowItDecides />
            </>
          )}
        </div>
