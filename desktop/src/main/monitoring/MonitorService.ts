...
import { InferenceEngine, type BehaviorSignal } from '../inference/InferenceEngine'
...
`.trim()
  ...
  ingestExtensionActivity(url: string, title: string, meta?: { description?: string; mediaPlaying?: boolean; headings?: string[]; behavior?: BehaviorSignal }): void {
    this.lastExtensionActivityTs = Date.now()
    if (!this.active) return
    if (title) this.currentTitle = title
    const behavior = meta?.behavior
    if (url !== this.currentUrl) {
      this.processUrl(url, title || this.currentTitle || '', 'extension', behavior)
    } else if (behavior) {
      // Same page, a mid-session behavioural update (dwell/scroll accrued since navigation):
      // re-run the hot path with the fresh behaviour — forced past the per-nav dedup — so
      // sustained engagement can escalate the verdict without re-logging a visit event.
      this.inference?.analyzeUrl(url, title || this.currentTitle || '', behavior, true)
    }
  }
  ...
  private processUrl(url: string, title: string, _source: 'powershell' | 'extension', behavior?: BehaviorSignal): void {
    this.currentUrl = url
    this.emit('url', url)
    ...
    this.inference?.analyzeUrl(url, title, behavior)
